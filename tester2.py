...
def get_hum(hr, curr):
	...
	while(humidity is None):
		...
		if (hr > 0):
			...
			get_hum(hr, curr)
	print('Successfully retrieve local humidity')
	print('Local humidity is: ', humidity)
...
def hum_thread():
	...
	while(not terminated):
		if((init) or (time.time() - start_time >= 3600)):
			...
			init = False
		time.sleep(5)
	print('Humidity Thread terminated')
...
def DHT_thread(lock):
	...
	while(not terminated):
		...
		weather_index = int(m)
		time.sleep(1)
	print('DHT Thread terminated')

def PIR_thread(lock):
	count = 0
	global l_status, fname, terminated
	print('PIR Thread starts')
	while(not terminated):
		time.sleep(2)
